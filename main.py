...
class AddTaskWindow(ctk.CTkToplevel):
    def __init__(self, master, on_add):
        ...
        ctk.CTkLabel(self, text=("عنوان:")).grid(row=0, column=1, padx=10, pady=10, sticky="e")
        self.title_entry = ctk.CTkEntry(self, placeholder_text=("مثلاً: خرید شیر"))
        self.title_entry.grid(row=0, column=0, padx=10, pady=10, sticky="ew")

        ctk.CTkLabel(self, text=("اولویت:")).grid(row=1, column=1, padx=10, pady=10, sticky="e")
        ...
        ctk.CTkLabel(self, text=("تاریخ موعد (YYYY-MM-DD):")).grid(row=2, column=1, padx=10, pady=10, sticky="e")
        self.due_entry = ctk.CTkEntry(self, placeholder_text=today_str())
        self.due_entry.grid(row=2, column=0, padx=10, pady=10, sticky="ew")

        btn = ctk.CTkButton(self, text=("افزودن"), command=self._do_add)
    ...
    def _do_add(self):
        ...
        if not title:
            messagebox.showwarning(("خطا"), ("عنوان نباید خالی باشد."))
            return
        try:
            print(due)
            task = Task(title=(title), priority=priority, due_date=due)
        except Exception:
            messagebox.showerror(("تاریخ نامعتبر", "لطفاً تاریخ را به صورت YYYY-MM-DD وارد کنید."))
            return
        self.on_add(task)
        self.destroy()
...
class FilterWindow(ctk.CTkToplevel):
    def __init__(self, master, on_filter):
        ...
        ctk.CTkLabel(self, text=("اولویت:")).grid(row=0, column=1, padx=10, pady=10, sticky="e")
        self.priority_combo = ctk.CTkComboBox(self, values=["", "Low", "Medium", "High"])
        self.priority_combo.set("")
        self.priority_combo.grid(row=0, column=0, padx=10, pady=10, sticky="ew")

        ctk.CTkLabel(self, text=("از تاریخ (YYYY-MM-DD):")).grid(row=1, column=1, padx=10, pady=10, sticky="e")
        self.from_entry = ctk.CTkEntry(self, placeholder_text=(""))
        ...
        self.to_entry = ctk.CTkEntry(self, placeholder_text=(""))
        self.to_entry.grid(row=2, column=0, padx=10, pady=10, sticky="ew")

        btn = ctk.CTkButton(self, text=("اعمال فیلتر"), command=self._do_filter)
        btn.grid(row=3, column=0, columnspan=2, padx=10, pady=18, sticky="ew")
...
class ReportWindow(ctk.CTkToplevel):
    def __init__(self, master, on_export):
        ...
        ctk.CTkLabel(self, text=("تاریخ (YYYY-MM-DD):")).grid(row=0, column=1, padx=10, pady=10, sticky="e")
        ...
        ctk.CTkLabel(self, text=("نام فایل خروجی:")).grid(row=1, column=1, padx=10, pady=10, sticky="e")
        ...
        btn = ctk.CTkButton(self, text=("تولید گزارش"), command=self._do_export)
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.today_btn = ctk.CTkButton(top, text="کارهای امروز", command=self.show_today)
        self.today_btn.grid(row=0, column=1, padx=8, pady=8)

        self.filter_btn = ctk.CTkButton(top, text=("جستجو/فیلتر"), command=self.open_filter)
        ...
        hdr = ctk.CTkLabel(center, text=("لیست کارها"))
        ...
        ctk.CTkButton(bottom, text=("افزودن"), command=self.open_add).grid(row=0, column=0, padx=6, pady=8, sticky="ew")
        ctk.CTkButton(bottom, text=("حذف"), command=self.remove_selected).grid(row=0, column=1, padx=6, pady=8, sticky="ew")
        ctk.CTkButton(bottom, text=("ذخیره"), command=self.save_data).grid(row=0, column=2, padx=6, pady=8, sticky="ew")
        ctk.CTkButton(bottom, text=("بارگذاری"), command=self.load_data).grid(row=0, column=3, padx=6, pady=8, sticky="ew")
        ctk.CTkButton(bottom, text=("گزارش روز"), command=self.open_report).grid(row=0, column=4, padx=6, pady=8, sticky="ew")
        ctk.CTkButton(bottom, text=("نمایش همه"), command=self.refresh_view).grid(row=0, column=5, padx=6, pady=8, sticky="ew")
    ...
    def format_task(self, idx, t: Task) -> str:
        ...
        return (f"{idx+1:>2}. [{done}] ({t.priority[:1]}) {t.title} | ایجاد: {t.created_date} | موعد: {t.due_date or '-'}")
    ...
    def refresh_view(self, items=None):
        ...
        self.status_label.configure(text=(f"تعداد تسک‌ها: {len(self.todo.list_tasks())}"))
    ...
    def remove_selected(self):
        ...
        if idx is None:
            messagebox.showinfo("حذف", ("ابتدا یک تسک را انتخاب کنید (روی خطش کلیک کنید)."))
            return
    ...
    def save_data(self):
        ...
        messagebox.showinfo("ذخیره شد", (f"تسک‌ها در فایل {DATA_FILE} ذخیره شدند."))

    def load_data(self):
        self.todo.load_from_file(DATA_FILE)
        self.refresh_view()
        messagebox.showinfo("بارگذاری شد", (f"تسک‌ها از فایل {DATA_FILE} بارگذاری شدند."))
    ...
    def export_report(self, date_str, filename):
        ...
        messagebox.showinfo("گزارش آماده شد", (f"گزارش در فایل {filename} ذخیره شد."))

    # --- Clock & notifications ---
    def tick_clock(self):
        self.clock_label.configure(text=(now_time_str()))
    ...
    def notify_due_tasks(self):
        ...
        msgs = []
        if due_today:
            msgs.append(f"{len(due_today)} تسک با موعد امروز")
        if overdue:
            msgs.append(f"{len(overdue)} تسک عقب‌افتاده")
        if msgs:
            text = " و ".join(msgs)
            # try:
            #     messagebox.showwarning("یادآوری", text)
            # except Exception:
            #     pass
            if HAS_PLYER:
                try:
                    ...
                    pass
        self.after(30000,self.notify_due_tasks)        
